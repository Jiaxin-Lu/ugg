...
class HandModel:
    ...
    def __init__(self, mjcf_path, mesh_path, contact_points_path, penetration_points_path, n_surface_points=0, device='cpu'):
        ...
        def build_mesh_recurse(body):
            if(len(body.link.visuals) > 0):
                ...
                areas[link_name] = tm.Trimesh(link_vertices.to(torch.float32).cpu().numpy(), link_faces.to(torch.float32).cpu().numpy()).area.item()
        ...
        for link_name in self.mesh:
            ...
            surface_points = surface_points.to(dtype=torch.float32, device=device)
    ...
    def set_parameters(self, hand_pose, contact_point_indices=None):
        ...
        if contact_point_indices is None:
            contact_point_indices = torch.arange(self.n_contact_candidates, device=self.device, dtype=torch.long).unsqueeze(0).repeat(B, 1)
    ...
    @torch.cuda.amp.autocast(False)
    def set_contact_point_indices(self, contact_point_indices):
        self.contact_point_indices = contact_point_indices
        batch_size, n_contact = contact_point_indices.shape
        self.contact_points = self.contact_candidates[self.contact_point_indices]
        link_indices = self.global_index_to_link_index[self.contact_point_indices]
    ...
    def get_all_trimesh_data(self):
        batch_size = self.global_translation.shape[0]
        data = [trimesh.Trimesh() for i in range(batch_size)]
        for link_name in self.mesh:
            v = self.current_status[link_name].transform_points(
                self.mesh[link_name]['vertices'])
            if len(v.shape) == 2:
                v = v.unsqueeze(0).repeat(batch_size, 1, 1)
            v_ = torch.bmm(v, self.global_rotation.transpose(2, 1)) + self.global_translation.unsqueeze(1)
            v = v_.detach().cpu()
            f = self.mesh[link_name]['faces'].detach().cpu()
            for i in range(batch_size):
                data[i] += trimesh.Trimesh(vertices=v[i], faces=f)
        return data
