...
class ObjectModel:
    def __init__(self, data_root_path, pc_num_points, device='cpu', dataset_name='dex'):
    ...
    def set_parameters(self, object_codes, object_pc=None, object_sc=None, object_rot=None):
        ...
        for i, (object_code, idx) in enumerate(self.object_code_list):
            ...
            if object_sc is None:
                grasp_data = np.load(os.path.join(self.data_root_path, "dexgraspnet", object_code+".npy"), allow_pickle=True)[idx]
        ...
        if len(self.object_mesh_list) > 1:
            assert len(self.object_mesh_list) == self.object_pc.shape[0]
            assert self.object_pc.shape[0] == self.object_sc.shape[0]
        elif len(self.object_mesh_list) == 1 and self.object_pc.shape[0] > 1:
            self.batch_size_each = self.object_pc.shape[0]
            assert self.object_pc.shape[0] == self.object_sc.shape[0]
        elif len(self.object_mesh_list) == 1 and self.object_sc.shape[0] > 1:
